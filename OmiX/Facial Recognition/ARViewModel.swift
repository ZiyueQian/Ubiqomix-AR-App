...
final class ARViewModel {
    ...
    private lazy var classificationRequest: VNDetectFaceRectanglesRequest = {
        do {
            let model = try VNCoreMLModel(for: Faces().model)
            let detectFaceRequest = VNDetectFaceRectanglesRequest(completionHandler: { (request, error) in
                if let face = request.results?.first as? VNFaceObservation {
                    ...
                    guard let worldCoordination = self.normalizeWorldCoord(boundingBox),
                        let FaceName = self.face?.name else { return }
                    guard let _ = self.cardDictionary[FaceName] else {
                        DispatchQueue.main.async{
                            self.currentNode = self.createCard(facePosition: worldCoordination, Face: self.face)
                        }
                        return
                    }
                }
    ...
    func processClassification (for request: VNRequest, error: Error?){
        ...
        if let bestResult = classifications.first(where: { result in result.confidence > 0.35}) {
            self.faces = dataController.loadJson("Faces")!
            self.faces?.forEach({ (Face) in
                if Face.name == bestResult.identifier {
                    self.face = Face
                }
            })
        }
    ...
    func createCard(facePosition: SCNVector3, Face: Face?) -> SCNNode? {
        guard let name = Face?.name else { return nil }
        guard let title = Face?.title else { return nil }
        guard let department = Face?.department else { return nil }
        ...
        let FaceName = createTextNode(string: "Name: \(name)", scale: Constants.nameTextScale)
        positionText(textNode: FaceName, card: card, heightLevel: 1.0)
        
        let FaceTitle = createTextNode(string: "Title: \(title)", scale: Constants.titleTextScale)
        positionText(textNode: FaceTitle, card: card, heightLevel: 2.0)
        
        let FaceDepartment = createTextNode(string: "Department: \(department)", scale: Constants.departmentTextScale)
        positionText(textNode: FaceDepartment, card: card, heightLevel: 3.0)
